...
Тем не менее в Bot API есть ограничения. Например, через него
нельзя [загружать большие файлы](../messages/sending.md#ограничения-размеров-фаилов) или получать старые
сообщения из истории чата.

Таких различий между Bot API и Telegram API немного, и я на этом сайте постарался упомянуть все важные различия.

От некоторых ограничений Bot API можно избавиться с
помощью [локального сервера](https://github.com/tdlib/telegram-bot-api). {#local-bot-api}
...
Кто-то пишет юзерботов, чтобы каждую минуту ставить текущее время себе на аватарку, кто-то — чтобы сохранять сообщения
из публичных групп и каналов.

В общем, с помощью юзерботов можно делать любые действия с Телеграм-аккаунтами, которые нельзя сделать с помощью ботов.

::: warning
Телеграм обычно не банит за использование юзерботов, но будьте осторожны: за спам ваш аккаунт могут ограничить 
или удалить.
:::
